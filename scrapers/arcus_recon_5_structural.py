#!/usr/bin/env python3
"""
PlanFind — Arcus reconnaissance script, round 5: structural link-finding.
...
async def test_council(browser, name: str, url: str):
    ...
    with open(f"/tmp/arcus_r5_{name.replace(' ', '_')}_00_homepage.html", "w", encoding="utf-8") as f:
        f.write(homepage_html)
    await page.screenshot(path=f"/tmp/arcus_r5_{name.replace(' ', '_')}_00_homepage.png", full_page=True)

    # --- STRUCTURAL approach: find the "Weekly lists" heading, then find
    # links that appear AFTER it in the DOM (siblings/descendants of its
    # containing section), rather than matching specific label text. ---
    clicked = False
    clicked_text = None
    try:
        heading = page.get_by_text("Weekly lists", exact=False).first
        if await heading.count() > 0:
            # Walk up to a reasonable container, then find all links within
            # it — this should catch Ashford's single link AND Epping
            # Forest's two links without needing to know the exact wording.
            container = heading.locator(
                "xpath=ancestor::*[self::div or self::section][1]"
            )
            links = container.get_by_role("link")
            link_count = await links.count()
            print(f"\nLinks found near 'Weekly lists' heading: {link_count}")
            ...
            if link_count > 0:
                clicked_text = (await links.first.inner_text()).strip()
                await links.first.click(timeout=5_000)
                clicked = True
                print(f"\n✓ Clicked first link found structurally: '{clicked_text}'")
        else:
    ...
    with open(f"/tmp/arcus_r5_{name.replace(' ', '_')}_01_after_click.html", "w", encoding="utf-8") as f:
        f.write(results_html)
    await page.screenshot(path=f"/tmp/arcus_r5_{name.replace(' ', '_')}_01_after_click.png", full_page=True)
    ...
    if indicators["Application Reference"] > 0 or indicators["Download as CSV"] > 0:
        try:
            ...
            csv_path = f"/tmp/arcus_r5_{name.replace(' ', '_')}.csv"
...
async def main():
    print(f"[{datetime.now(timezone.utc).isoformat()}] Arcus recon round 5 — structural link-finding + SSL fix\n")
