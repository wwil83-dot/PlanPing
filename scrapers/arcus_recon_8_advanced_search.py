...
async def test_council(browser, name: str, url: str):
    ...
    with open(f"/tmp/arcus_r8_{name.replace(' ', '_')}_00_homepage.html", "w", encoding="utf-8") as f:
        f.write(homepage_html)
    await page.screenshot(path=f"/tmp/arcus_r8_{name.replace(' ', '_')}_00_homepage.png", full_page=True)
    ...
    with open(f"/tmp/arcus_r8_{name.replace(' ', '_')}_01_advanced_form.html", "w", encoding="utf-8") as f:
        f.write(form_html)
    await page.screenshot(path=f"/tmp/arcus_r8_{name.replace(' ', '_')}_01_advanced_form.png", full_page=True)

    # --- Try filling in a 14-day date range, matching the Idox scraper's
    # own DAYS_BACK=14 default, using whatever "Valid date from"/"Valid
    # date to" fields exist. Field labels varied slightly in the
    # screenshots ("Valid date from" on Manchester) so try a couple of
    # label variants.
    ...
    for label_variant in ["Valid date from", "Date from", "Received date from"]:
        try:
            ...
            if await date_input.count() > 0:
                ...
                print(f"✓ Filled '{label_variant}' with {date_from_str}")
                filled = True
                break
        except Exception:
            continue
    ...
    if not filled:
        print(f"⚠ Could not find a 'date from' field to fill using known label variants.")
    ...
    with open(f"/tmp/arcus_r8_{name.replace(' ', '_')}_02_search_results.html", "w", encoding="utf-8") as f:
        f.write(results_html)
    await page.screenshot(path=f"/tmp/arcus_r8_{name.replace(' ', '_')}_02_search_results.png", full_page=True)
    ...
    if indicators["Application Reference"] > 0 or indicators["Reference"] > 3 or indicators["Download as CSV"] > 0:
        ...
        try:
            ...
            csv_path = f"/tmp/arcus_r8_{name.replace(' ', '_')}.csv"
...
async def main():
    print(f"[{datetime.now(timezone.utc).isoformat()}] Arcus recon round 8 — Advanced Search portability test\n")
