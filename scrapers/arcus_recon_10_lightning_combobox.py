...
async def test_council(browser, name: str, url: str):
    ...
    with open(f"/tmp/arcus_r10_{name.replace(' ', '_')}_00_homepage.html", "w", encoding="utf-8") as f:
        f.write(homepage_html)
    await page.screenshot(path=f"/tmp/arcus_r10_{name.replace(' ', '_')}_00_homepage.png", full_page=True)
    ...
    with open(f"/tmp/arcus_r10_{name.replace(' ', '_')}_01_advanced_form.html", "w", encoding="utf-8") as f:
        f.write(form_html)
    await page.screenshot(path=f"/tmp/arcus_r10_{name.replace(' ', '_')}_01_advanced_form.png", full_page=True)

    # --- FIX (round 10): round 10's select_option() failed on all three
    ...
    try:
        ...
        if await category_field.count() > 0:
            ...
            await page.screenshot(
                path=f"/tmp/arcus_r10_{name.replace(' ', '_')}_category_open.png",
                full_page=True,
            )
    ...
    # own DAYS_BACK=14 default. Fill BOTH from/to this time (round 10 only
    ...
    with open(f"/tmp/arcus_r10_{name.replace(' ', '_')}_02_search_results.html", "w", encoding="utf-8") as f:
        f.write(results_html)
    await page.screenshot(path=f"/tmp/arcus_r10_{name.replace(' ', '_')}_02_search_results.png", full_page=True)
    ...
    if indicators["Application Reference"] > 0 or indicators["Reference"] > 3 or indicators["Download as CSV"] > 0:
        ...
        try:
            ...
            csv_path = f"/tmp/arcus_r10_{name.replace(' ', '_')}.csv"
...
async def main():
    print(f"[{datetime.now(timezone.utc).isoformat()}] Arcus recon round 10 — Advanced Search portability test\n")
