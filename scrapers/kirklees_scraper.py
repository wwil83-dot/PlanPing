#!/usr/bin/env python3
"""
PlanFind — Kirklees Council scraper (2026-08-31).

Real, confirmed evidence backing the search-form design — see
kirklees_councils.py. Results-page structure is UNCONFIRMED — see that
...
from kirklees_councils import COUNCIL_DB_IDS, SEARCH_URL
...
def _parse_results_page_generic(html: str) -> list[dict]:
    """UNCONFIRMED structure — generic defensive parsing. Kirklees
    application references are typically YYYY/NNNNN (e.g. 2026/94481,
    per a real filename seen during web research:
    filedownload.aspx?application_number=2021/94481)."""
    ...
    for table in tables:
        ...
        for row in rows[1:]:
            ...
            ref_match = re.search(r"\b(19|20)\d{2}/\d{4,6}\b", row_text)
            if not ref_match:
                continue
            link = row.find("a")
    ...
    if not apps:
        for link in soup.find_all("a", href=True):
            ...
            ref_match = re.search(r"\b(19|20)\d{2}/\d{4,6}\b", text)
            if ref_match:
                apps.append({
                    "reference": ref_match.group(0),
                    "raw_row_text": text,
...
async def scrape() -> list[dict]:
    ...
    async with async_playwright() as pw:
        ...
        try:
            ...
            await page.fill("#txtDateFrom", start_str, timeout=5_000)
            await page.fill("#txtDateTo", end_str, timeout=5_000)
            submit = page.locator("#ctl00_ctl00_cphPageBody_cphContent_btnAdvSearch")
...
async def main():
    print(f"[{datetime.now(timezone.utc).isoformat()}] PlanFind Kirklees scraper")
    ...
    if cid is None:
        print(f"ERROR: {COUNCIL_NAME} has a placeholder (None) DB id in "
              f"kirklees_councils.py. Run the INSERT_SQL there, look up "
    ...
    if ok:
        ...
        await _supa_patch_council(cid, {
            "coverage_source": "kirklees_bespoke",
